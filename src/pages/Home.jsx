...
function cafeInitial(name) { return (name || '?').trim()[0].toUpperCase(); }
function cafeGrad(idx) {
      ...
      <section className="sec">
        ...
        <div className="strip">
          {games.slice(0, 10).map(g => (
            <Link key={g.id} to={`/games/${g.id}`} className="gtile">
              <div className="gimg" style={{ background: gameGrad(g.bggId) }}>
                {gameEmoji(g.bggId)}
              </div>
              <p>{g.title}</p>
              <span>★ {g.bggRating != null ? Number(g.bggRating).toFixed(1) : '—'}</span>
            </Link>
          ))}
        </div>
      </section>
