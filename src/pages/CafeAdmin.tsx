...
import type { CafeDetail, BggSearchResult } from '../types';

interface LocationForm {
  name: string; address: string; about: string; hours: string;
  dayPass: string; contact: string; mapUrl: string; fanpage: string;
}

type TFunction = (key: string) => string;

const TABS: { id: string; label: (t: TFunction) => string }[] = [
...
export default function CafeAdmin() {
  ...
  const [adding,      setAdding]      = useState<number | null>(null);  // bggId being added
  ...
  const [locForm,   setLocForm]   = useState<LocationForm | null>(null); // initialized on first open
  ...
  // ── BGG search + add ─────────────────────────────────────────────────────────
  async function searchBgg(e: React.FormEvent) {
    e.preventDefault();
    if (!bggQ.trim()) return;
    setBggLoading(true);
    setBggResults([]);
  ...
  async function addGame(bggId: number) {
    if (!cafe) return;
    setAdding(bggId);
    try {
  ...
  // ── Library management ───────────────────────────────────────────────────────
  async function toggleFeatured(cg: { id: string; featured?: boolean }) {
    try {
      ...
      {tab === 'location' && locForm && (
        <div className="card">
          ...
          <form onSubmit={saveLocation} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            ] as { key: keyof LocationForm; label: string; type: string; placeholder?: string }[]).map(({ key, label, type, placeholder }) => (
              <div key={key} className="fld">
                <label>{label}</label>
                <input
                  type={type}
                  value={locForm[key]}
                  onChange={e => setLocForm(f => f ? ({ ...f, [key]: e.target.value }) : f)}
            ...
            <div className="fld">
              ...
              <textarea
                value={locForm.about}
                onChange={e => setLocForm(f => f ? ({ ...f, about: e.target.value }) : f)}
                rows={4}
