...
function cafeGrad(name: string | undefined): string { return GR[(name?.charCodeAt(0) ?? 0) % GR.length]; }
function avColor(name: string | undefined): string  { return AV[(name?.charCodeAt(0) ?? 0) % AV.length]; }
const starStr = (n: number) => '★★★★★'.slice(0, Math.round(n)) + '☆☆☆☆☆'.slice(0, 5 - Math.round(n));

// Returns true/false/null (unknown). Computes against Vietnam time UTC+7.
// hours formats: "Daily 9:00–23:00", "Tue–Sun 13:00–23:00"
function computeOpenStatus(hours: string | null | undefined): boolean | null {
